...
#define LOG_FILE_TOTAL_MEMORY       256 *1024                               //log文件总存储大小
#define LOG_FILE_SINGLE_MEMORY      LOG_FILE_TOTAL_MEMORY / LOG_FILE_COUNT  //log文件单个文件大小
#define LOG_FILE_PRE_NAME           "70mai_mcu_log_"                        //log文件名称前半部分
...
namespace maix {
    CMCULogManageStorage::CMCULogManageStorage(CModule *objModule)
    ...
    CMCULogManageStorage::~CMCULogManageStorage()
    {
    }

    mxbool CMCULogManageStorage::init()
    {
        if(!m_objModule)
        {
            return mxfalse;
        ...
        if (!m_objModule->getConfig("LOG_STORAGE_CONFIG", "MCU_LOG_PATH", m_strLogDir))
        {
            return mxfalse;
        }       
    ...
    mxbool CMCULogManageStorage::unInit()
    {
        if(m_pLogFd)
        {
            fflush(m_pLogFd);
    ...
    mxbool CMCULogManageStorage::pushLogData(std::string &logData)
    {
        if(!m_bInit)
        {
            return false;
    ...
    void CMCULogManageStorage::popLogData(std::string &logData)
    {
        m_qLogList.pop(logData);
    }

    void CMCULogManageStorage::run()
    {
        m_bRun = mxtrue;
        while (m_bRun)
        {
    ...
    void CMCULogManageStorage::writeLogToFile(std::string logData)
    {
        changeWriteFile();
        if(m_pLogFd)
        {
    ...
    /**
     * @brief 切换写日志文件
     * 思路:以文件名来判断当前到哪个文件，需要写的文件命名中增加tmp，例如 70mai_mcu_log_1.log  70mai_mcu_log_2_tmp.log
     * 
     */
    void CMCULogManageStorage::changeWriteFile()
    {
    ...
    int CMCULogManageStorage::checkTmpFilePosition(std::string tmpFilePath)
    {
    ...
    std::string CMCULogManageStorage::montageLogPath(std::string strDir,std::string strPreName,int iNum,std::string strBehindName)
    {
        char buf[100] = { 0 };
    ...
    size_t CMCULogManageStorage::getFileSize(const char* fileName)
    {
        if(fileName == NULL)
        {
            return 0;
    ...
    mxbool CMCULogManageStorage::isFileExit(std::string filePath)
    {
        return (access(filePath.c_str(),F_OK) == 0);
    }

    mxbool CMCULogManageStorage::flushLogCache()
    {
        if(m_pLogFd)
        {
            fflush(m_pLogFd);
