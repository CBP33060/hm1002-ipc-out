...
public:
    ...
    virtual void init(const char *portName,
                      int baudRate,
                      maix::Parity parity,
                      maix::DataBits dataBits,
                      maix::StopBits stopbits,
                      maix::FlowControl flowControl,
                      unsigned int readBufferSize) = 0;

    /**
     * @brief Set the Operate Mode object 设置串口操作模式
     *
     * @param operateMode [in] the operate mode 串口操作模式 {@link maix::OperateMode}
     */
    virtual void setOperateMode(maix::OperateMode operateMode);
     ...
     */
    virtual bool openPort(int isNoBuf) = 0;
    /**
     * @brief close 关闭串口
     *
     */
     ...
     */
    int connectReadEvent(maix::CSerialPortListener *event);
     ...
     */
    virtual void setParity(maix::Parity parity) = 0;
    /**
     * @brief Get the Parity object 获取校验位
     *
     * @return return parity 返回校验位 {@link maix::Parity}
     */
    virtual maix::Parity getParity() const = 0;
     ...
     */
    virtual void setDataBits(maix::DataBits dataBits) = 0;
    /**
     * @brief Get the Data Bits object 获取数据位
     *
     * @return return dataBits 返回数据位 {@link maix::DataBits}
     */
    virtual maix::DataBits getDataBits() const = 0;
     ...
     */
    virtual void setStopBits(maix::StopBits stopbits) = 0;
    /**
     * @brief Get the Stop Bits object 获取停止位
     *
     * @return return stopbits 返回停止位 {@link maix::StopBits}
     */
    virtual maix::StopBits getStopBits() const = 0;
     ...
     */
    virtual void setFlowControl(maix::FlowControl flowControl) = 0;
    /**
     * @brief Get the Flow Control object 获取流控制
     *
     * @return maix::FlowControl
     */
    virtual maix::FlowControl getFlowControl() const = 0;
...
protected:
    ...
    maix::OperateMode m_operateMode;                     ///< operate mode 串口操作类型
    ...
    maix::IMutex *p_mutex;                               ///< mutex 互斥锁
    maix::CSerialPortListener *p_readEvent;              ///< read event 读取事件
    maix::ITimer<maix::CSerialPortListener> *p_timer; ///< read timer 读取定时器
private:
};
