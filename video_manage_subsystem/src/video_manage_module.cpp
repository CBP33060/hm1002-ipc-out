...
namespace maix {
	CVideoManageModule::CVideoManageModule(std::string strGUID, 
		std::string strName)
	...
	CVideoManageModule::~CVideoManageModule()
	{
	}

	mxbool CVideoManageModule::init()
	{
	...
	mxbool CVideoManageModule::unInit()
	{
	
		return mxbool();
	}

	mxbool CVideoManageModule::initServer()
	{
		...
		for (int j = 0; j < iComServerConfigNum; j++)
		{
			...
			if (!isClientExist(strName))
			{
				...
				else if (strComType.compare("RCF_EVENT") == 0)
				{
					...
					{
						CVideoManageRemoteEventServer * objVideoManageRemoteEventServer =
							new CVideoManageRemoteEventServer(this);

						if (!objVideoManageRemoteEventServer)
							return mxfalse;

						objVideoManageRemoteEventServer->init();
						...
						objServerEndPoint->init(serverConfig, 
							objVideoManageRemoteEventServer);
	...
	mxbool CVideoManageModule::initConnectModule()
	{
	...
	mxbool CVideoManageModule::addVideoChannel(std::string strName, 
		CVideoSourceInputServer * objVideoSourceInputServer)
	{
	...
	std::string CVideoManageModule::openVideo(std::string strParam)
	{
		...
		if (jsonRoot)
		{
			...
			else
			{
				...
				if(iLiveStream == 1)
				{
					...
					m_iLiveStreamNum++;
					logPrint(MX_LOG_DEBUG, "m_iLiveStream add:%d", m_iLiveStreamNum);
				}
			}
		...
		CModule *module = dynamic_cast<CModule *>(imodule.get());
		std::shared_ptr<CVideoManageChannel> objVideoManageChannel =
			m_mapVideoChn[strChannelName];

		if (objVideoManageChannel)
		{
			std::shared_ptr<CComRCFClientEndPoint> objClientEndPoint(
				new CComRCFClientEndPoint(this));
	...
	std::string CVideoManageModule::closeVideo(std::string strParam)
	{
		cJSON *jsonRoot = cJSON_Parse(strParam.c_str());
		std::string strModuleGUID;
		std::string strChannelName;
		...
		if (jsonRoot)
		{
			...
			else
			{
				...
				{
					std::unique_lock<std::mutex> lock(m_mutexLiveStreamNum);
					if(m_iLiveStreamNum > 0)
					{
						m_iLiveStreamNum--;
						logPrint(MX_LOG_DEBUG, "m_iLiveStream del:%d", m_iLiveStreamNum);
					}
				}
			}
		...
		if (m_mapVideoChn.count(strChannelName) != 0)
		{
			std::shared_ptr<CVideoManageChannel> objVideoManageChannel =
				m_mapVideoChn[strChannelName];
			if (objVideoManageChannel)
			{
				objVideoManageChannel->close(strModuleGUID, strServerName);
			}
			else
			{
				return  procResult(std::string("500"),"",
	...
	std::string CVideoManageModule::configVideo(std::string strParam)
	{
		return std::string();
	}

	std::string CVideoManageModule::resetVideo(std::string strParam)
	{
		return std::string();
	}

	std::string CVideoManageModule::enterLowPower(std::string strParam)
	{
		logPrint(MX_LOG_DEBUG, "Enter low power mode ,live stream:%d", m_iLiveStreamNum);
		if(m_iLiveStreamNum > 0)
		{
				return procResult(std::string("500"), "", "video manage is in live stream");
		}
		else
		{
			{
			...
			return procResult(std::string("200"), "", "video manage enter low power");
		}
			
	}
	
	std::string CVideoManageModule::procResult(std::string code,
	...
	mxbool CVideoManageModule::initLowPower()
	{
	...
	void CVideoManageModule::lowPowerRun()
	{
		while (1)
		{
			std::unique_lock<std::mutex> lock(m_mutexLowPower);
	...
	std::string CVideoManageModule::lowPowerProc()
	{
		cJSON *jsonRoot = cJSON_CreateObject();
