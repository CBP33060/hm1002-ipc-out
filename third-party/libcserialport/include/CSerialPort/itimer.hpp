...
public:
...
#endif
    {
        if (prctl(PR_SET_NAME, "threadFun") != 0) {
            printf("prctl\n");
            return NULL;
        }
        // printf("WCQ==%s==PID===%d\n",__func__,getpid());
    ...
    void startOnce(unsigned int timeoutMs, T *pclass, void (T::*pmemfun)(const char *, unsigned int), const char *portName, unsigned int readBufferLen)
    {
        ...
        p_class = pclass;
        p_memfun = pmemfun;
        maix::IUtils::strncpy(m_portName, portName, PORT_NAME_MAX_LEN);
        ...
        maix::i_thread_create(&handle, NULL, threadFun, (void *)this);
    }
...
#else
    maix::IConditionVariable m_cvStop;
    maix::IConditionVariable m_cv;
    maix::IMutex m_stopMutex;
#endif

    maix::i_thread_t handle;
