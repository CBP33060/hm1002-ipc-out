...
std::vector<maix::SerialPortInfo> getPortInfoListMac()
{
    // https://developer.apple.com/documentation/iokit/communicating_with_a_modem_on_a_serial_port
    maix::SerialPortInfo m_serialPortInfo;
    std::vector<maix::SerialPortInfo> portInfoList;
    ...
    while (serialPort = IOIteratorNext(serialPortIterator))
    {
        ...
        maix::IUtils::strncpy(m_serialPortInfo.portName, device_path, MAXPATHLEN);
        maix::IUtils::strncpy(m_serialPortInfo.description, "", 1);
        maix::IUtils::strncpy(m_serialPortInfo.hardwareId, "", 1);
...
std::vector<maix::SerialPortInfo> getPortInfoList()
{
    std::vector<maix::SerialPortInfo> portInfoList;
#ifdef I_OS_LINUX
    ...
    {
        maix::IUtils::strncpy(m_serialPortInfo.portName, portList[i].c_str(), 256);
        maix::IUtils::strncpy(m_serialPortInfo.description, "", 1);
        maix::IUtils::strncpy(m_serialPortInfo.hardwareId, "", 1);
        portInfoList.push_back(m_serialPortInfo);
    }
...
std::vector<maix::SerialPortInfo> CSerialPortInfoUnixBase::availablePortInfos()
{
