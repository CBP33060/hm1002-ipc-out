...
#include "cJSON.h"
namespace maix {
	CDevManageRemoteEventServer::CDevManageRemoteEventServer(
		CDevManageModule * objDevManageModule)
		: m_objDevManageModule(objDevManageModule)
	{
	}
	...
	mxbool CDevManageRemoteEventServer::init()
	{
		return mxbool();
	}

	mxbool CDevManageRemoteEventServer::unInit()
	{
		return mxbool();
	}

	std::string CDevManageRemoteEventServer::eventProc(std::string strMsg)
	{
		std::string strEvent;
		std::string strParam;
		cJSON *jsonRoot = cJSON_Parse(strMsg.c_str());
		logPrint(MX_LOG_DEBUG, "dev event proc:%s", strMsg.c_str());
		if (jsonRoot)
		{
		...
		if (m_objDevManageModule)
		{
			return m_objDevManageModule->remoteEventServerProc(
				strEvent, strParam);
		}
		else
		{
			return procResult(std::string("500"), "",
				std::string("dev manage module not exist"));
		}
	}

	std::string CDevManageRemoteEventServer::procResult(std::string code, 
