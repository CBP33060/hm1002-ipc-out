...
namespace maix {
	CCenterManageRemoteEventServer::CCenterManageRemoteEventServer(
		CCenterManageModule * objCenterManageModule)
		: m_objCenterManageModule(objCenterManageModule)
	{
	}
	...
	mxbool CCenterManageRemoteEventServer::init()
	{
		return mxbool();
	}

	mxbool CCenterManageRemoteEventServer::unInit()
	{
		return mxbool();
	}

	std::string CCenterManageRemoteEventServer::eventProc(
		std::string strMsg)
	{
		std::string strEvent;
		std::string strParam;
		...
		if (m_objCenterManageModule)
		{
			return m_objCenterManageModule->remoteEventServerProc(
				strEvent, strParam);
		}
		else
		{
			return procResult(std::string("500"), "",
				std::string("center manage module not exist"));
		}
	}

	std::string CCenterManageRemoteEventServer::procResult(
