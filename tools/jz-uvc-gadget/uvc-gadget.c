...
static int GetFWParaConfig(const char *key, char *value)
{
	int ret = 0;
	char cmd[128] = {0};
	snprintf(cmd, sizeof(cmd), "fw_printenv user %s", key);
	FILE *pf = popen(cmd, "r");
	char temp[512] = {0};
...
{
	char cmd[128] = {0};
	snprintf(cmd, sizeof(cmd), "fw_setenv user %s %d", key,value);
...
static int uvc_video_process(struct uvc_device *dev)
{
        ...
        int ret;
        // printf("WCQ====uvc_video_process====\n");
        if (!dev->is_streaming)
                return 0;
        // printf("WCQ====uvc_video_process aAAAAa====\n");
        CLEAR(ubuf);
        ...
        // printf("WCQ====uvc_video_process bbbbbbbb====ret:%d\n",ret);
        if (ret < 0)
                return ret;
        ...
        // printf("WCQ====uvc_video_process cccccccccccc====ret:%d\n",ret);
        if (ret < 0)
                return ret;
...
// #endif
        ...
        while (1) {
                ...
                if (FD_ISSET(udev->uvc_fd, &efds))
                {
                        ...
                        if(udev->uvc_shutdown_requested)
                        {
                            if (udev->is_streaming) {
                                    ...
                                    udev->is_streaming = 0;
                                    printf("WCQ======is_streaming======\n");
                            }
                            uvc_close(udev);
                            // printf("udev->imgsize:%d\n", udev->imgsize);
                            ret = uvc_open(&udev, uvc_devname);
                            udev->uvc_devname = uvc_devname;
                                /* Set parameters as passed by user. */
                    #if defined SENSOR_WIDTH
                ...
                if (FD_ISSET(udev->uvc_fd, &dfds))
                        if(uvc_video_process(udev) == -1){
                            // printf("WCQ======ADASDASDASD======\n"); 
                        }
        }
        // printf("WCQ===============END\n");
        if (udev->is_streaming) {
