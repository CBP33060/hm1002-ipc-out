...
int GetFWParaConfig(char *strKey, char *strValue)
{
	int bRet = 0;
	char acCmd[128] = {0};
	snprintf(acCmd, sizeof(acCmd), "fw_printenv user %s", strKey);
	FILE *pFile = popen(acCmd, "r");
	char acValue[512] = {0};
	if(fgets(acValue, sizeof(acValue), pFile) != NULL)
	{
		acValue[strlen(acValue) - 1] = 0;
		strcpy(acValue,strValue);
		bRet = 0;
	}
	else
	{
...
{
	printf("WCQ=======sample_system_init start\n");
	...
	if( GetFWParaConfig("wdr_mode",wdr) )
	{
		if(wdr[0] == '1')
		{
			...
			sensor_info[0].default_boot = 1;
			printf("================ IMPISP_TUNING_OPS_MODE_ENABLE ================\n");
		}
		else
		{
			sensor_info[0].default_boot = 0;
		}
	}
	...
	// if(ret < 0){
	// 	IMP_LOG_ERR(TAG, "IMP_Encoder_SetIvpuBsSize failed\n");
	// 	return -1;
	// }

	ret = IMP_Encoder_SetAvpuBsShare(1,1846592);
	if(ret < 0){
		IMP_LOG_ERR(TAG, "IMP_Encoder_SetAvpuBsShare failed\n");
		return -1;
	}
	...
	if(ret < 0){
		IMP_LOG_ERR(TAG, "IMP_ISP_Tuning_GetModuleControl failed\n");
		return -1;
	}
...
int sample_encoder_init()
{
	...
	for (i = 0; i <  2; i++) {
		if (chn[i].enable) {
			...
			printf("rcMode:%d.uTargetBitRate:%d\n", S_RC_METHOD,uTargetBitRate);
			...
			ret = IMP_Encoder_RegisterChn(chn[i].index, 0);
			if (ret < 0) {
...
int sample_jpeg_init()
{
	...
	for (i = 0; i <  FS_CHN_NUM; i++) {
		if (chn[i].enable) {
			imp_chn_attr_tmp = &chn[i].fs_chn_attr;
			memset(&channel_attr, 0, sizeof(IMPEncoderChnAttr));
			ret = IMP_Encoder_SetDefaultParam(&channel_attr, IMP_ENC_PROFILE_JPEG, IMP_ENC_RC_MODE_CBR,
					imp_chn_attr_tmp->picWidth, imp_chn_attr_tmp->picHeight,
					imp_chn_attr_tmp->outFrmRateNum, imp_chn_attr_tmp->outFrmRateDen, 0, 0, 85, 2000);
...
int save_stream_by_name(char *stream_prefix, int idx, IMPEncoderStream *stream)
{
	int stream_fd = -1;
...
void *get_frame_thread(void *args)
{
	...
	if (PIX_FMT_NV12 == chn[index].fs_chn_attr.pixFmt) {
		sprintf(framefilename, "/tmp/mnt/sdcard/frame%dx%d_%d.nv12", chn[index].fs_chn_attr.picWidth, chn[index].fs_chn_attr.picHeight, index);
	} else {
...
int sample_get_frame_22()
{
	int ret;
	unsigned int i = 2;
	pthread_t tid;

			ret = pthread_create(&tid, NULL, get_frame_thread, (void *)i);
			if (ret < 0) {
				IMP_LOG_ERR(TAG, "Create ChnNum%d get_frame_thread failed\n", chn[i].index);
				return -1;
			}
...
void *get_video_stream_thread(void *args)
{
	...
	printf("0101010101%d\r\n",val);
	chnNum = val & 0xffff;
	printf("000000000000chnNum::::%d\r\n",chnNum);
	encType = (val >> 16) & 0xffff;
	printf("111111111111encType::::::%d\r\n",encType);
...
int sample_get_video_stream()
{
	...
	pthread_t tid[4];
			...
			arg = (((chn[0].payloadType >> 24) << 16) | 0);
			ret = pthread_create(&tid[0], NULL, get_video_stream_thread, (void *)arg);
			if (ret < 0) {
			...
			// arg = 0;
			// arg = (((chn[1].payloadType >> 24) << 16) | 1);
		
			// ret = pthread_create(&tid[1], NULL, get_video_stream_thread, (void *)arg);
			// if (ret < 0) {
			// 	IMP_LOG_ERR(TAG, "Create ChnNum%d get_video_stream_thread failed\n", (chn[i].payloadType == IMP_ENC_PROFILE_JPEG) ? (4 + chn[i].index) : chn[i].index);
			// }
		
			// arg = 0;
			// arg = (((chn[1].payloadType >> 24) << 16) | 2);
		
			// ret = pthread_create(&tid[2], NULL, get_video_stream_thread, (void *)arg);
			// if (ret < 0) {
			// 	IMP_LOG_ERR(TAG, "Create ChnNum%d get_video_stream_thread failed\n", (chn[i].payloadType == IMP_ENC_PROFILE_JPEG) ? (4 + chn[i].index) : chn[i].index);
			// }
			

			// ret = pthread_create(&tid[3], NULL, get_frame_thread, (void *)2);
			// if (ret < 0) {
			// 	IMP_LOG_ERR(TAG, "Create ChnNum%d get_frame_thread failed\n", chn[i].index);
			// 	return -1;
			// }
...
void *get_h265_jpeg_stream_thread(void *args)
{
	int val, i, chnNum, ret;
	char stream_path[64];
	IMPEncoderEncType encType;
