...
namespace maix {
	COTAManageRemoteEventServer::COTAManageRemoteEventServer(
		COTAManageModule * objOTAManageModule)
		: m_objOTAManageModule(objOTAManageModule)
	{
	}
	...
	mxbool COTAManageRemoteEventServer::init()
	{
		return mxtrue;
	}

	mxbool COTAManageRemoteEventServer::unInit()
	{
		return mxtrue;
	}

	std::string COTAManageRemoteEventServer::eventProc(std::string strMsg)
	{
		std::string strEvent;
		std::string strParam;
		...
		if (m_objOTAManageModule)
		{
			return m_objOTAManageModule->remoteEventServerProc(
				strEvent, strParam);
		}
		else
		{
			return procResult(std::string("500"), "",
				std::string("ota manage module not exist"));
		}
	}

	std::string COTAManageRemoteEventServer::procResult(
