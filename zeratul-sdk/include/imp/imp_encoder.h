 ...
 * 设置avpu共享码流buffer属性
 *
 * @param[in] enAvpuBs 是否使能avpu共享码流buffer
 * @param[in] avpuBsSize avpu码流buffer的大小
 *
 * @retval 0 成功
 * @retval 非0 失败
 *
 * @remarks avpuBsSize=0 共享码流buffer为默认大小
 * @remarks 使能共享码流buffer功能存在丢帧的可能性
 *
 * @attention 使能AvpuBs必须在IMP_Encoder_CreateChn之前
 * @attention 不使能必须在IMP_Encoder_DestroyChn之后
 */
 ...
 * @param[in]  uMaxSameSenceCnt 一个GOP长度 = uGopLength * uMaxSameSenceCnt, 默认sdk设置为2
...
/**
 * @fn int IMP_Encoder_StartRecvPicNoIDR(int encChn)
 *
 * 开启编码Channel接收图像,主要用于zeratul aov功能
 * 和IMP_Encoder_StartRecvPic区别在于,只会开启编码,但是不会强制申请I帧
 *
 * @param[in] encChn 编码Channel号,取值范围: [0, @ref NR_MAX_ENC_CHN - 1]
 *
 * @retval 0 成功
...
int IMP_Encoder_StartRecvPicNoIDR(int encChn);
/**
 ...
 *
 * @param[in] encChn 编码Channel号,取值范围: [0, @ref NR_MAX_ENC_CHN - 1]
 * @param[in] iGopLength 编码GopLength属性
 *
 * @retval 0 成功
 * @retval 非0 失败
 *
...
/**
 * @fn int IMP_Encoder_SetAvpuJpegQp(int encChn, int jpegQp)
 *
 * 动态设置avpu JpegQp
 *
...
typedef struct {
	IMPEncoderEncType type;	//编码类型
	IMPEncoderRcMode mode;	//码率控制模式
	uint16_t frameRate;		//帧率
	uint16_t gopLength;		//GOP长度
	uint32_t targetBitrate;	//目标码率
	uint32_t maxBitrate;	//最大码率
	int16_t initQp;			//初始化Qp
	uint16_t minQp;			//最小Qp
	uint16_t maxQp;			//最大Qp
	uint32_t maxPictureSize;//最大帧大小
} IMPEncoderYuvIn;
