 ...
 */
typedef struct {
   int threshold1;           /** 临界点default 50000 **/
   float ratio;             /** 比率 0.3~0.5 **/
   int threshold2;           /** 临界点default 100000 **/
   int mode;                /** 模式 1 或 0 ，0 标准模式，1 反向模式 threshold 之上能量不变，threshold 之下能量消减 **/
   float supPoint;          /** 起始抑制频段 取值 0 ~ 1.0，该值乘以最大频段代表起始抑制频段**/
   float supExp;            /** 抑制参数 值越大抑制越强 **/
} IMPAudioDrcConfig;
...
/*For Zeratul*/
/**
 * @fn int IMP_AI_EnableDrc(IMPAudioIOAttr *attr, IMPAudioDrcConfig drcConfig)
 *
 * 启用Ai动态范围压缩功能.
 *
 * @param[in] attr 需要进行的音频属性.
 * @param[in] drcConfig 动态范围压缩的参数配置.
 *
 * @retval 0 成功.
 * @retval 非0 失败.
 *
...
/**
 * @fn void IMP_ADEC_Set_G726_BitRate(int bitrate)
 *
 * 设置音频编码g726比特率
 * @param[in] bitrate : 2:16K 3:24K 4:32k 5:40k
 *
 * @remarks 无.
 *
 * @attention 无.
 */
void IMP_ADEC_Set_G726_BitRate(int bitrate);

/**
 * @fn void IMP_AENC_Set_G726_BitRate(int bitrate)
 *
 * 设置音频解码g726比特率
 * @param[in] bitrate : 2:16k  3:24k 4:32k 5:40k
 *
 ...
 * @attention 无.
 */
void IMP_AENC_Set_G726_BitRate(int bitrate);
#ifdef __cplusplus
#if __cplusplus
}
#endif
